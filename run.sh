...
shell2http -cgi -export-vars=ALLOW_ORIGIN -form -port 9991 \
  /run 'echo $ALLOW_ORIGIN; ffmpeg -y -i burger.mp4 -i touch.mp4 -i yes.png -filter_complex "${v_filter// /;}" -map "[out]" -c:v libx264 -x264opts "bframes=1:keyint=1:min-keyint=1:no-scenecut" out/$v_file; echo $v_file;' \
  ...
  /yes 'echo $ALLOW_ORIGIN; cat yes.png' \
